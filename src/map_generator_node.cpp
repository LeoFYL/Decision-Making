...
bool srv_update_callback(world_percept_assig3::UpdateObjectList::Request  &req,
         ...
         //这个是update_object_list这个survice的调回函数，它处理将新对象添加到 map_objs_ 列表的请求。
{
...
/**
     * @brief Callback function for the service that sends the info of a target object 
     * in the scene. This service could be use, for example, 
     * by the controller node to find the target position
     *
     * @param Request name of the requested object. 
     * Use "all" to get the list of all the available objects.
            ...
            for (const auto& pair : map_objs_)
             {
                ...
                res.objects.pose.push_back(pair.second);
            }
