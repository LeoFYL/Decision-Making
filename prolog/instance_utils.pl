...
get_class(Class):-
	 % Use the existing getClassPath/2 predicate to get the full class path
    getClassPath(Class, ClassPath),
    % Check if the class already exists
    ( rdf_has(ClassPath, rdf:type, owl:'Class') ->
        % If the class exists, do nothing and print a message
        write('Class already exists: '), write(Class), nl
		fail
    ;
        % If the class does not exist, assert it in the ontology
        rdf_assert(ClassPath, rdf:type, owl:'Class'),
        write('New class asserted: '), write(Class), nl
    ).
